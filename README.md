...
| `/[locale]/dashboard/profile` | **PROFILE & ACCOUNT SETTING / 公司檔案**（2026-07；英文介面於 2026-08 更名）：唯讀展示會員 `OnboardingProfile`（公司/需求兩區，slug 經 `Auth.options.*` 轉 label），附「編輯」→ `/dashboard/account`；未填顯示引導卡。 |
...
| `/[locale]/dashboard/playbooks`（+`[slug]`） | **FUNDRAISING / 知識手冊（2026-07；英文介面於 2026-08 更名）**：ROLL ON 募資／成長方法論指南，登入即可看。一份＝一個 `content/playbooks/<slug>.json`（`pnpm ingest:playbook` 把 PDF 經 Claude 轉雙語 JSON）。**每個紅色標題＝一個 segment**；詳情頁 `PlaybookReader` 分段渲染 + 每段「標為已讀」+「全部／未讀／已讀」filter（狀態存 `User.playbookReads`），列表頁顯示各份已讀進度。**雙用**：同內容餵 Nova AI（`get_playbook`）。 |
| `/[locale]/dashboard/quiz` | **FOUNDER QUIZ / 創辦人測驗（2026-07；2026-08 由「本期問答 Fortnightly Q&A」更名——⚠️ 與 DASHBOARD 每日提醒中指向 `/quiz`（創辦人決策風格測驗）的同名卡片撞名，待釐清）**：每兩週一批選擇題，題庫來自 playbook 各段（ingest 時每段產 2 題）。以會員註冊日為錨**即時算第幾個雙週**（`src/lib/playbook/quiz.ts`，無 cron），全部題庫循環出題；`PlaybookQuizClient` 作答 → server 端重算分數＋對錯＋解析，每期一筆存 `PlaybookQuizAttempt`。**不寄 email、不排程**、**不**綁 onboarding。 |
